    ...
    axios
      ...
      .then((response) => {
        if (response.data.success) {
          ...
          setFormErrorMessage(
            "Study Group Thumnail Upload Error! Check out your input"
          );
        }
      })
      .catch((err) => {
    ...
    return (
      <Formik
        ...
        validationSchema={Yup.object().shape({
          manager: Yup.string(),
          title: Yup.string().required("Study Group title is required"),
          description: Yup.string()
            .required("Study Group description is required")
            .min(10, "description must be at least 10 characters"),
          contactInfo: Yup.string().required("Contact infomation is required"),
          capacity: Yup.number().required("Study Group capacity is required"),
          filePath: Yup.string(),
        })}
            ...
            dispatch(editStudyGroup(dataToSubmit))
              .then((response) => {
                if (response.payload.success) {
                  ...
                  setFormErrorMessage(
                    "Study Group Edit Error! Check out your input"
                  );
                }
              })
              .catch((err) => {
                  ...
                  <Form.Item label="Thumnail">
                    ...
                    >
                      ...
                      {StudyGroupFilePath && (
                        ...
                        >
                          <img
                            src={`http://localhost:3001/${StudyGroupFilePath}`}
                            alt="thumanail_lecuture"
                          />
                        </div>
                      )}
                    </div>
