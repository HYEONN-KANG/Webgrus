...
function StudyGroupContentsList(props) {
  const [formErrorMessage, setFormErrorMessage] = useState("");
  const StudyGroupContents = props.StudyGroupContents;
  ...
  const onDelete = (event, postId) => {
    ...
    axios.post("/api/StudyGroupContents/delete", variable).then((response) => {
      if (response.data.success) {
        message.warning("Cotents deleted!");
        window.location.reload();
      } else {
        message.error(
          "Cotents deletion Error! Please contact the site manager"
        );
        window.location.reload();
      }
    });
        ...
        {StudyGroupContents.map((contents, index) => {
          return (
            ...
            >
              ...
              <br />
              <div
                style={{ display: "grid", gridTemplateColumns: "5fr 1fr 1fr" }}
              >
                <Meta
                ...
                <Formik
                  ...
                  onSubmit={(values, { setSubmitting }) => {
                    setTimeout(() => {
                      ...
                      axios
                        .post("/api/studygroupContents/edit", dataToSubmit)
                        .then((response) => {
                          if (response.data.success) {
                            ...
                            setFormErrorMessage(
                              "Cotents Edit Error! Check out your input"
                            );
                          }
                        })
                        .catch((err) => {
...
export default withRouter(StudyGroupContentsList);
