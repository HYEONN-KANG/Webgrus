...
function StudyGroupContentsTab(props) {
  ...
  useEffect(() => {
    ...
    axios.post("/api/studygroupContents/get", variable).then((response) => {
      if (response.data.success) {
        ...
        message.error(
          "Study Group List Error! Please contact the site manager"
        );
  ...
  if (
    user.userData._id === props.ThisStudyGroup.manager._id &&
    StudyGroupContents
  ) {
    contentsList = (
    ...
    postStudyGroupContent = (
      <StudyGroupContentsPost
        ThisStudyGroup={props.ThisStudyGroup}
        user={user}
      />
    );
