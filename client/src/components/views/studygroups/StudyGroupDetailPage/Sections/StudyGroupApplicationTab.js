...
function StudyGroupApplicationTab(props) {
  ...
  useEffect(() => {
    ...
    axios
      .post(
        "/api/studygroupApplication/getStudyGroupApplicants",
        applicantsVariable
      )
      .then((response) => {
        if (response.data.success) {
          ...
          message.error(
            "Study Group Infomation Error! Please contact the site manager"
          );
    ...
    axios
      .post("/api/studygroupApplication/getAppliedStudyGroup", appliedVariable)
      .then((response) => {
        if (response.data.success) {
    ...
    console.log(StudyGroupApplicants);
    axios
      .post("/api/studygroups/updateApplicationStatus", updateVariable)
      .then((response) => {
        if (response.data.success) {
          const variable = {
            studygroupId: ThisStudyGroup._id,
          ...
          axios
            ...
            .then((response) => {
              if (response.data.success) {
                if (
                  ThisStudyGroup.applicationPeriod !=
                  response.data.studygroupDetail.applicationPeriod
                ) {
  ...
  const onApply = () => {
    ...
    if (AppliedStudyGroup) {
      axios
        .post("/api/studygroupApplication/cancleApply", applyVariable)
        .then((response) => {
          if (response.data.success) {
      ...
      axios
        .post("/api/studygroupApplication/toApply", applyVariable)
        .then((response) => {
          if (response.data.success) {
            setStudyGroupApplicants(StudyGroupApplicants + 1);
            setAppliedStudyGroup(!AppliedStudyGroup);
    ...
    if (localStorage.getItem("userId") === ThisStudyGroup.manager._id) {
              ...
              return (
                <Col xs={24} key={index}>
                  <div
                    style={{
                      display: "grid",
                      gridTemplateColumns: "0.5fr 1fr 2fr",
                    }}
                  >
                    <div />
                    <Meta
                      avatar={<Avatar src={applicants.ApplicantInfo.image} />}
                      title={applicants.ApplicantInfo.name}
                      description=""
                    />
                    <h3 style={{ position: "relative", bottom: "4px" }}>
                      Contact - {applicants.ApplicantInfo.email} / 0
                      {applicants.ApplicantInfo.phonenumber}
                    </h3>
                  </div>
                </Col>
              );
      ...
      return (
        ...
        >
          ...
          <h2>
            {StudyGroupApplicants} / {ThisStudyGroup.capacity}
          </h2>
          <p />
          <Button
            style={{ height: "auto", minWidth: "275px" }}
            onClick={onApply}
          >
            <h3
              style={{
            ...
            {AppliedStudyGroup ? (
              <h3 style={{ marginBottom: "5px", fontWeight: 550 }}>
                신청 완료
              </h3>
            ) : (
      ...
      return (
        ...
        >
          ...
          <Button style={{ height: "auto", minWidth: "275px" }}>
            <h3 style={{ color: "coral", fontWeight: 550, marginTop: "5px" }}>
              모집 마감
            </h3>
            {AppliedStudyGroup ? (
              <h3 style={{ marginBottom: "5px", fontWeight: 550 }}>
