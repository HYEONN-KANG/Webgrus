...
const FreeBoard = ({ user }) => {
  ...
  const searchWriting = () => {
    axios
      .get("/api/freeBoard/search", { params: keyword }) // 글 목록 요청 -> 페이지별 글 나누기
      .then((res) => {
        setWrites(res.data);
      });
  };
      ...
      <div className={styles.container}>
        <section className={styles.freeBoard}>
          <div className={styles.header}>
            <div className={styles.freeBoardTitle}>자유게시판</div>
          ...
          {editWrite.status && (
            <EditWrite write={editWrite.write} editWriting={editWriting} />
          )}
          {addWrite && (
            <AddWrite
          ...
          {addWrite || (
            <ul
              className={`${
                editWrite.status ? styles.hidden : styles.writingList
              }`}
            >
              {writes.writes &&
          ...
          {addWrite || (
            <div
              className={`${styles.pages} ${
                editWrite.status ? styles.hidden : styles.none
              }`}
            >
              {writes.pages &&
